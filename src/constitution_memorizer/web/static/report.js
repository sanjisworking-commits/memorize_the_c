/**
 * Report an issue dialog controller (Browse Article).
 * Shared openDialog({ mode }) shell — only mode "report" is wired in this PR.
 */
(function () {
  "use strict";
  ...
  function clearFieldErrors() {
    ...
    document.querySelectorAll(".rc-input[aria-invalid]").forEach(function (el) {
      el.removeAttribute("aria-invalid");
    });
  }

  function setFieldError(name, message) {
    var err = $('[data-rc-error-for="' + name + '"]');
    var field =
      name === "issue_type"
        ? $("[data-rc-issue-type]")
        : name === "description"
  ...
  function showSuccess(reportId) {
    ...
    if (idEl) {
      var shortId = String(reportId || "").replace(/-/g, "").slice(0, 8);
  ...
  function resetFormView() {
    ...
    if (submit) {
      ...
      submit.textContent = MODE_COPY.report.primary;
  ...
  function openDialog(opts) {
    ...
    if (mode !== "report") {
      // Contact / gate reserved for a later PR.
      return;
    }
    var overlay = $("[data-report-overlay]");
    ...
    announce("Report an issue dialog opened");
    renderTurnstile();

    window.setTimeout(function () {
      var issueType = $("[data-rc-issue-type]");
      if (issueType) issueType.focus();
      else {
  ...
  async function onSubmit(event) {
    ...
    try {
      var resp = await fetch("/api/report-issue", {
        method: "POST",
      ...
      if (resp.status === 201 && data && data.success) {
        showSuccess(data.report_id);
        return;
      }

      var detail = (data && data.detail) || "";
      if (typeof detail !== "string") detail = "Unable to submit report right now.";
      ...
      if (resp.status === 400 || resp.status === 503) {
        ...
        setBanner(detail || "Sign in to report an issue.");
        announce("Sign in required");
      } else if (resp.status === 422) {
        setBanner("Please check the highlighted fields.");
        announce("Validation failure");
      } else {
        setBanner("Unable to submit report right now. Your text is still here — try again in a moment.");
        announce("Submission failed");
      }
    } catch (err) {
      setBanner(
